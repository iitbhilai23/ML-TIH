...
//   // --- MAP LOCATION FILTERING (COUNTRY/STATE) ---
...
//   // --- MAP DISPLAY LOCATIONS (FILTERED BY DISTRICT CODE) ---
//   // Used for the Map Markers representing static locations
...
//   // --- ROBUST FILTERING LOGIC FOR TRAINING LOCATIONS ---
//   // This maps the District Code (from filter) -> District Name -> Matches against Training String Name
//   const filteredTrainingLocations = useMemo(() => {
//     if (!trainingLocations.length) return [];

//     // 1. Get selected district name from the dropdown list (districts array)
//     const selectedDistrictObj = districts.find(d => String(d.district_cd) === String(filters.district_cd));
    
//     // 2. Clean the expected district name
//     const selectedDistrictName = selectedDistrictObj?.district_name?.trim().toLowerCase();
...
//       if (filters.district_cd) {
//         // Get the district name from the training record and clean it
//         const trainingDistName = loc.district ? String(loc.district).trim().toLowerCase() : '';

//         // Compare the cleaned strings
//         matchDistrict = (trainingDistName === selectedDistrictName);
//       }

//       // --- Block Matching ---
//       const selectedBlockObj = blocks.find(b => String(b.block_cd) === String(filters.block_cd));
//       const selectedBlockName = selectedBlockObj?.block_name?.trim().toLowerCase();
      
//       let matchBlock = true;
//       if (filters.block_cd) {
//          const trainingBlockName = loc.block ? String(loc.block).trim().toLowerCase() : '';
//          matchBlock = (trainingBlockName === selectedBlockName);
//       }
...
//           summary={data} 
//           viewData={viewData} 
//           trainingLocations={filteredTrainingLocations} // Passing the FILTERED list here
//           mapLocations={mapDisplayLocations} 
//           focusTarget={mapFocusPoint}
//           totalTrainingsCount={filteredTrainingLocations.length} // Passing the COUNT here
//         />
//       )}
...
// const SummaryTab = ({ summary, viewData, trainingLocations, mapLocations, focusTarget, totalTrainingsCount }) => (
...
//       {/* Total Trainings now uses totalTrainingsCount which is derived from filteredTrainingLocations */}
//       <StatCard title="Total Trainings" value={totalTrainingsCount || 0} icon={BookOpen} gradient={THEME.gradients.kpiA} />
//       <StatCard title="Total Trainers" value={summary?.total_trainers || 0} icon={User} gradient={THEME.gradients.kpiB} />
//       <StatCard title="Total Participants" value={summary?.total_participants} icon={Users} gradient={THEME.gradients.kpiC} />
...
const Dashboard = () => {
  ...
  // New state to hold the global/static summary data
  ...
  // --- FETCH GLOBAL STATS (Static) ---
  // This runs only once on mount to get the static numbers for Trainings, Trainers, Participants
  useEffect(() => {
  ...
  // Calculate the static count for trainings from the full list
  const staticTotalTrainings = trainingLocations.length;
  ...
  return (
    <div style={{ padding: '10px 15px 15px 15px', display: 'flex', flexDirection: 'column', gap: THEME.gap.xs, minHeight: '100vh', background: THEME.bgGradient, overflowX: "hidden" }}>
      ...
      {activeTab === 'summary' && (
        <SummaryTab 
          summary={data} 
          globalSummary={globalSummary} // Passing global summary for static cards
          viewData={viewData} 
          trainingLocations={filteredTrainingLocations} 
          mapLocations={mapDisplayLocations} 
          focusTarget={mapFocusPoint}
          totalTrainingsCount={staticTotalTrainings} // Passing static training count
        />
      )}
      {activeTab === 'detailed' && <DetailedTab viewData={viewData} />}
    </div >
...
// Updated to use globalSummary for static data and summary for dynamic data
const SummaryTab = ({ summary, globalSummary, trainingLocations, mapLocations, focusTarget, totalTrainingsCount }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: THEME.gap.sm }}>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: THEME.gap.sm }}>
      {/* Total Trainings: Static (passed from parent) */}
      <StatCard title="Total Trainings" value={totalTrainingsCount || 0} icon={BookOpen} gradient={THEME.gradients.kpiA} />
      
      {/* Total Trainers: Static (from globalSummary) */}
      <StatCard title="Total Trainers" value={globalSummary?.total_trainers || 0} icon={User} gradient={THEME.gradients.kpiB} />
      
      {/* Total Participants: Static (from globalSummary) */}
      <StatCard title="Total Participants" value={globalSummary?.total_participants || 0} icon={Users} gradient={THEME.gradients.kpiC} />
      
      {/* Total Locations: Dynamic (from summary which updates with filters) */}
      <StatCard title="Total Locations" value={summary?.total_locations || 0} icon={House} gradient={THEME.gradients.kpiD} />
    </div>
    <div style={{ width: '100%', height: '560px', marginTop: '8px', borderRadius: '20px', overflow: 'hidden' }}>
      <TraineeLocationMap 
