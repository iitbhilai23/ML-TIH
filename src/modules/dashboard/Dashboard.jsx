...
const Dashboard = () => {
  ...
  return (
    <div style={{ padding: '5px 10px 10px 10px', display: 'flex', flexDirection: 'column', gap: THEME.gap.xs, minHeight: '100vh', background: THEME.bgGradient, overflowX: "hidden" }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', px: 1, mb: 0.5 }}>
        <div style={{ ...THEME.glass, width: '100%', maxWidth: '1200px', justifyContent: 'center', padding: '6px 10px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: THEME.gap.xs, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: THEME.gap.xs, paddingRight: THEME.pad.sm, borderRight: '1px solid #f3f4f6', color: THEME.primary, fontWeight: '700', letterSpacing: '0.05em', textTransform: 'uppercase', fontSize: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: THEME.gap.xs }}>
            ...
            <input type="date" name="start_date" style={{...THEME.input, height: '36px', fontSize: '0.8rem'}} onChange={handleFilterChange} value={filters.start_date} />
            <span style={{ color: '#94a3b8', fontWeight: '600', fontSize: '0.75rem', margin: `0 ${THEME.gap.xs}` }}>to</span>
            <input type="date" name="end_date" style={{...THEME.input, height: '36px', fontSize: '0.8rem'}} onChange={handleFilterChange} value={filters.end_date} />
          </div>
...
const TraineeLocationMap = ({ trainingLocations }) => {
  ...
  // FIX: Lock body scroll when modal is open to prevent background jump
  useEffect(() => {
    if (selectedTraining) {
  ...
  // FIX: Added 'leaflet-interactive' class to ensure click events register on first try
  const createCustomIcon = (count, color = '#7b3f99') =>
    L.divIcon({
      className: 'custom-div-icon leaflet-interactive', 
      html: `
        <div style="
            width: 36px;
            height: 36px;
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ position: 'absolute', top: '10px', left: '10px', zIndex: 1000, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(8px)', padding: '8px 12px', borderRadius: '10px', boxShadow: '0 4px 15px rgba(0,0,0,0.08)', border: '1px solid rgba(255,255,255,0.8)', display: 'flex', alignItems: 'center', gap: '8px', pointerEvents: 'auto' }}>
        <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#9647bb', border: '2px solid white' }}></div>
        <div style={{ fontSize: '0.8rem', fontWeight: '700', color: '#1e293b' }}>Total Trainings ({totalTrainings})</div>
      </div>
      ...
      <MapContainer style={{ width: '100%', height: '100%' }} zoomControl={false} doubleClickZoom={false}>
        ...
        {Object.values(groupedLocations).map((location, i) => {
          ...
          return (
            <Marker
              ...
              eventHandlers={{
                click: (e) => {
                  // FIX: Stop propagation to prevent map from moving/processing the click
      ...
      {Array.isArray(selectedTraining) && selectedTraining.length > 0 && (
        ...
        >
          ...
          >
            <button onClick={() => setSelectedTraining(null)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}><X size={18} color="#334155" /></button>
            <div style={{ padding: '24px' }}>
              <h3 style={{ margin: '0 0 16px 0', color: '#1e293b', fontSize: '1.2rem', fontWeight: '700' }}>Trainings at This Location ({selectedTraining.length})</h3>
              <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '14px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'start', gap: '8px' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: '600', color: '#334155', fontSize: '0.95rem' }}>{selectedTraining[0]?.location_details?.village || 'N/A'}</div>
                    <div style={{ fontSize: '0.85rem', color: '#64748b' }}>{[selectedTraining[0]?.location_details?.block, selectedTraining[0]?.location_details?.district].filter(Boolean).join(', ')}</div>
                  </div>
                </div>
              </div>
              {selectedTraining.map((training, index) => (
                <div key={training.id || index} style={{ padding: '14px', borderRadius: '14px', background: '#ffffff', border: '1px solid #e5e7eb', marginBottom: '12px' }}>
                  <div style={{ fontWeight: '700', color: '#1e293b', marginBottom: '4px' }}>{training.subject_name || 'N/A'}</div>
                  <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '8px' }}>Trainer: {training.trainer_name || 'Unknown'}</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ ...getStatusStyle(training.status), padding: '4px 10px', borderRadius: '12px', fontSize: '0.7rem', fontWeight: '700', textTransform: 'uppercase' }}>{training.status}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: '#475569' }}><Users size={14} />{training.total_participants || 0}</div>
                  </div>
                </div>
              ))}
            </div>
...
// ===== MAP BOUNDS ADJUSTER (Fixed Zoom) =====
...
const SummaryTab = ({ summary, viewData, locationsData, trainingLocations }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: THEME.gap.sm }}>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: THEME.gap.sm }}>
